...
static void s256t(S256 *c, const uint8_t *d){
    ...
    for(;i<64;i++)
        w[i]=(R32(w[i-2],17)^R32(w[i-2],19)^(w[i-2]>>10))+w[i-7]+
             (R32(w[i-15],7)^R32(w[i-15],18)^(w[i-15]>>3))+w[i-16];
...
static void s256u(S256 *c, const uint8_t *d, size_t l){
    ...
    if(used){
        ...
        memcpy(c->b+used,d,fr); s256t(c,c->b); d+=fr; l-=fr;
    }
...
static void s256f(S256 *c, uint8_t *dg){
    ...
    s256u(c,lb,8);
    for(int i=0;i<8;i++){
        dg[i*4]=(c->s[i]>>24)&0xff; dg[i*4+1]=(c->s[i]>>16)&0xff;
        dg[i*4+2]=(c->s[i]>>8)&0xff; dg[i*4+3]=c->s[i]&0xff;
    }
}
...
/* Returns HTTP status code, -1 on network error */
static int http_upload(const char *filepath, const char *filename){
    ...
    char *fbuf=malloc((size_t)fsz); if(!fbuf){ fclose(fp); return -1; }
    if((long)fread(fbuf,1,(size_t)fsz,fp)!=fsz){ fclose(fp); free(fbuf); return -1; }
    fclose(fp);
    ...
    long body_len=(long)p1len+fsz+(long)p2len;
    ...
    int fd=tcp_connect();
    if(fd<0){ free(fbuf); fprintf(stderr,"[!] Cannot connect to %s:%d\n",SERVER_HOST,SERVER_PORT); return -1; }
...
static void upload_file(const char *path){
    ...
    sleep(SETTLE_SECS);   /* let download/copy finish */
    ...
    if(code==200||code==201){
        ...
        /* Server says duplicate — mark locally so we don't retry */
        printf("[=] Duplicate (server): %s\n",name);
        pthread_mutex_lock(&g_hlock);
        cache_add(digest);
        pthread_mutex_unlock(&g_hlock);
    } else {
