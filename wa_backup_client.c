 ...
 *
 * Compile (Linux):
 *   gcc -Wall -Wextra -O2 -std=c11 -o wa_backup_client wa_backup_client.c -lpthread
 *
 * Run:
 ...
 * ── NETWORK SETUP ─────────────────────────────────────────────────────────
 *
 *   Raspberry Pi  (server)  : 192.168.100.6   (your LAN)
 *   Laptop/PuTTY            : 192.168.100.7   (same LAN)
 *   Client machine          : 10.252.106.127  (different network)
 *
 *   Because the client is on a DIFFERENT network (10.x.x.x) from the
 *   Raspberry Pi (192.168.100.x), a direct TCP connection is NOT possible.
 *
 *   SOLUTION — use Tailscale (free, takes ~5 minutes to set up):
 *
 *   Step 1 — On Raspberry Pi (via PuTTY):
 ...
 *     Log in with the SAME account as the Pi
 *
 *   Step 3 — Update SERVER_HOST below with the Pi's Tailscale IP (100.x.x.x)
 *     Then recompile and run the client.
 *
 *   If both machines are on the SAME LAN later, just set SERVER_HOST back
 *   to "192.168.100.6" and it will work without Tailscale.
 * ──────────────────────────────────────────────────────────────────────────
 */
 ...
 *
 *  IF client is on a DIFFERENT network (your case: 10.252.106.127):
 *    SERVER_HOST = "<Tailscale IP of Raspberry Pi>"   e.g. "100.x.x.x"
 *    Get it by running:  tailscale ip -4   on the Raspberry Pi
 * ─────────────────────────────────────────────────────────────────────── */
#define SERVER_HOST   "192.168.100.6"   /* <-- Change to Tailscale IP if on different network */
#define SERVER_PORT   5050
#define API_KEY       "wa_secret_key_2024"   /* Must match server's api_key argument */
#define DEVICE_ID     "whatsapp_client"       /* Label shown in server logs */

/*
 * Folders to watch — add the actual path where WhatsApp saves media.
 * Common locations:
 *   Linux Desktop WhatsApp:  /home/YOUR_USERNAME/Downloads
 *   Android via MTP mount:   /run/user/1000/gvfs/.../WhatsApp/Media
 *
 * IMPORTANT: Replace "your_username" with your actual Linux username.
 * Run   whoami   in a terminal to find it.
 */
static const char *WATCH_PATHS[] = {
    "/home/your_username/Downloads",          /* <-- Replace your_username */
    /* Add more paths below if needed, e.g.: */
    /* "/home/your_username/WhatsApp/Media",  */
    NULL   /* sentinel — keep last, do not remove */
};

/* Local file that caches already-uploaded SHA-256 hashes (avoids re-uploads) */
#define HASH_CACHE_FILE  "/tmp/.wa_backup_hashes"
/* Seconds to wait after a new file appears (lets download/copy finish) */
#define SETTLE_SECS      3
/* Max upload size (bytes) — 200 MB */
...
static void s256t(S256 *c, const uint8_t *d){
    ...
    for(;i<64;i++)
        w[i]=(R32(w[i-2],17)^R32(w[i-2],19)^(w[i-2]>>10))
            +w[i-7]
            +(R32(w[i-15],7)^R32(w[i-15],18)^(w[i-15]>>3))
            +w[i-16];
    a=c->s[0]; b=c->s[1]; cc=c->s[2]; dd=c->s[3];
...
static void s256u(S256 *c, const uint8_t *d, size_t l){
    ...
    if(used){
        size_t fr = 64-used;
        if(l<fr){ memcpy(c->b+used,d,l); return; }
        memcpy(c->b+used,d,fr);
        s256t(c,c->b);
        d+=fr; l-=fr;
    }
    for(size_t i=0; i+64<=l; i+=64) s256t(c,d+i);
    size_t rem = l % 64;
    if(rem) memcpy(c->b, d+(l/64)*64, rem);
}
...
static void s256f(S256 *c, uint8_t *dg){
    ...
    size_t u = c->c & 63;
    p[0]=0x80;
    if(u<56) s256u(c,p,56-u);
    else     s256u(c,p,64-u+56);
    uint64_t bits = c->c * 8;
    uint8_t lb[8];
    for(int i=7;i>=0;i--){ lb[i]=(uint8_t)(bits&0xff); bits>>=8; }
    s256u(c,lb,8);
    for(int i=0;i<8;i++){
        dg[i*4+0]=(c->s[i]>>24)&0xff;
        dg[i*4+1]=(c->s[i]>>16)&0xff;
        dg[i*4+2]=(c->s[i]>>8)&0xff;
        dg[i*4+3]=c->s[i]&0xff;
    }
}
...
static void cache_load(void){
    FILE *fp = fopen(HASH_CACHE_FILE,"r");
    if(!fp) return;
    char line[70];
    ...
    fclose(fp);
    printf("[cache] Loaded %d previously uploaded hashes\n", g_hash_count);
}

static int cache_has(const char *h){
    for(int i=0;i<g_hash_count;i++)
        if(!strcmp(g_hashes[i],h)) return 1;
    return 0;
}
...
static void cache_add(const char *h){
    ...
    FILE *fp = fopen(HASH_CACHE_FILE,"a");
    if(!fp) return;
    fprintf(fp,"%s\n",h);
    fclose(fp);
}
...
static int tcp_connect(void){
    ...
    hints.ai_family   = AF_UNSPEC;       /* supports both IPv4 and IPv6 (Tailscale uses IPv4) */
    hints.ai_socktype = SOCK_STREAM;

    char port_s[16];
    snprintf(port_s,sizeof(port_s),"%d",SERVER_PORT);

    int rc = getaddrinfo(SERVER_HOST, port_s, &hints, &res);
    if(rc != 0){
        fprintf(stderr,"[!] getaddrinfo(%s): %s\n", SERVER_HOST, gai_strerror(rc));
        return -1;
    }
    ...
    for(struct addrinfo *rp = res; rp; rp = rp->ai_next){
        ...
        /* 30-second send/receive timeout */
...
/* ── HTTP multipart upload — returns HTTP status code, or -1 on error ─── */
static int http_upload(const char *filepath, const char *filename){
    ...
    long fsz = ftell(fp);
    rewind(fp);
    if(fsz <= 0 || fsz > MAX_UPLOAD_SIZE){
        fprintf(stderr,"[!] File size invalid (%ld bytes): %s\n", fsz, filepath);
        fclose(fp);
        return -1;
    }
    char *fbuf = malloc((size_t)fsz);
    if(!fbuf){ fclose(fp); return -1; }
    if(fread(fbuf,1,(size_t)fsz,fp) != (size_t)fsz){
    ...
    /* Multipart part 1: device_id field + file field header */
    char part1[1024];
    ...
    int fd = tcp_connect();
    if(fd < 0){ free(fbuf); return -1; }

    if(send_all(fd,hdr,   (size_t)hlen)  < 0 ||
       send_all(fd,part1, (size_t)p1len) < 0 ||
       send_all(fd,fbuf,  (size_t)fsz)   < 0 ||
       send_all(fd,part2, (size_t)p2len) < 0){
        fprintf(stderr,"[!] Send error for %s\n", filename);
        free(fbuf); close(fd); return -1;
    }
    free(fbuf);
...
static void upload_file(const char *path){
    ...
    printf("[^] Uploading %s (%lld KB) to %s:%d...\n",
           name, (long long)st.st_size/1024, SERVER_HOST, SERVER_PORT);

    int code = http_upload(path, name);

    if(code==200 || code==201){
        printf("[OK] Uploaded: %s (HTTP %d)\n", name, code);
        pthread_mutex_lock(&g_hlock);
        cache_add(digest);
        pthread_mutex_unlock(&g_hlock);
    } else if(code == -1){
        printf("[!!] Network error uploading: %s\n", name);
        printf("     Check that server is running on %s:%d\n",SERVER_HOST,SERVER_PORT);
    } else {
        printf("[!!] Server rejected: %s (HTTP %d)\n", name, code);
    }
}
...
static void full_scan(void){
    ...
    for(int i=0; WATCH_PATHS[i]; i++){
        DIR *d = opendir(WATCH_PATHS[i]);
        if(!d){
            printf("[~] Path not found (skipping): %s\n", WATCH_PATHS[i]);
            continue;
        }
        struct dirent *ent;
        while((ent=readdir(d))){
            if(ent->d_name[0]=='.') continue;
            char full[MAX_PATH_SZ];
            snprintf(full,sizeof(full),"%s/%s",WATCH_PATHS[i],ent->d_name);
            upload_file(full);
        }
        closedir(d);
    }
    printf("[OK] Full scan complete.\n\n");
}

/* ── inotify watcher ──────────────────────────────────────────────────────── */
#define MAX_WATCHES 64
static int  g_wd_to_id[MAX_WATCHES];
static char g_wd_path[MAX_WATCHES][MAX_PATH_SZ];
...
static void watch_loop(void){
    ...
    for(int i=0; WATCH_PATHS[i] && active<MAX_WATCHES; i++){
        int wd = inotify_add_watch(ifd, WATCH_PATHS[i],
                                   IN_CLOSE_WRITE|IN_MOVED_TO);
        if(wd<0){
            printf("[~] Cannot watch (skipping): %s\n", WATCH_PATHS[i]);
            continue;
        }
        /* Store wd -> path mapping */
        for(int j=0; j<MAX_WATCHES; j++){
            if(g_wd_to_id[j]==0){
                g_wd_to_id[j] = wd;
                strncpy(g_wd_path[j], WATCH_PATHS[i], MAX_PATH_SZ-1);
                break;
            }
        }
        printf("[W] Watching: %s\n", WATCH_PATHS[i]);
    ...
    if(active==0){
        printf("[!!] No valid watch paths. Edit WATCH_PATHS in the source.\n");
        close(ifd);
        return;
    }

    printf("\nWatcher active. Ctrl+C to stop.\n\n");
    ...
    while(g_running){
        ...
        while(p < evbuf+n){
            ...
            if(ev->len>0 && !(ev->mask & IN_ISDIR)){
                char base[MAX_PATH_SZ]={0};
                for(int j=0; j<MAX_WATCHES; j++){
                    if(g_wd_to_id[j] == ev->wd){
                        strncpy(base, g_wd_path[j], MAX_PATH_SZ-1);
                        break;
                    }
                }
                if(base[0]){
...
static void check_status(void){
    ...
    if(fd<0){
        ...
        printf("     Make sure the server is running on the Raspberry Pi.\n");
        printf("     If on different networks, use Tailscale (see top of file).\n");
        return;
    }
    char req[512];
    ...
    send_all(fd,req,(size_t)n);

    char resp[2048]={0};
    size_t rlen=0;
    while(rlen<sizeof(resp)-1){
        ssize_t k = read(fd,resp+rlen,1);
        if(k<=0) break;
        rlen++;
    }
    close(fd);
    ...
    else     printf("No response body received.\n");
}
